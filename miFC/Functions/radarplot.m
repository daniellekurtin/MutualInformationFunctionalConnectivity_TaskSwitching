function f=radarplot(R,Lable,MAX,LineColor,FillColor,LineStyle,LevelNum)
    ...
    if length(FillColor)>=m
        for i=1:m
            if sum(strcmpi({'y' 'm' 'c' 'r' 'g' 'b' 'w' 'k' },FillColor(i)))
                hold on;
                %F=patch(X(i,:),Y(i,:),'Color',FillColor,'LineStyle','none');
                patch('Faces',X,'Vertices',Y,'FaceColor','green')
                set(F,'FaceAlpha',0.3)
            end
        end
    end
